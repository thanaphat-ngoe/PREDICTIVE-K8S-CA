...
class DecisionEngine:
    def __init__(self, cores_per_node = 4.0, max_workers = 2, min_workers = 1):
        ...
        self.scale_in_percent  = 0.90  # Scale In when predicting < 90% of capacity (after removing 1 node) 
    ...
    def decide(self, predicted_cores, current_workers, pending_pods, current_cpu_usage, current_cpu_req):
        ...
        if pending_pods > 0:
            ...
            # Predictive Logic
            cores_after_scale_in = (current_workers - 1) * self.cores_per_node # 4vCores
            ...
            if predicted_cores > scale_out_threshold: # Ramp Up Phase: Always trust AI-Engine by provisioning more resources when AI predicts high demand 
                intent = "SCALE_OUT"
                reason = f"[AI] Need {predicted_cores:.2f} Cores (> {scale_out_threshold:.2f} limit)"
            elif current_cpu_req < 0.85 * cores_after_scale_in and predicted_cores < scale_in_threshold: # Ramp Down Phase: (เพิ่ม Asymmetric Logic เช็คโลกความเป็นจริง)
                intent = "SCALE_IN"
                reason = f"[Safe Scale-In] Predict ({predicted_cores:.2f}) & Current ({current_cpu_req:.2f}) are safe for {current_workers-1} nodes"                    
            else:
                intent = "DO_NOTHING"
                reason = f"[Wait] AI Predicts {predicted_cores:.2f}, but Current Req is still high ({current_cpu_req:.2f} >= {0.85 * cores_after_scale_in:.2f})"
