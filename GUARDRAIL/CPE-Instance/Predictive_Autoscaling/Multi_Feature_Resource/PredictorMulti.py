...
# 🌟 ฟังก์ชันดึงข้อมูลดัดแปลงใหม่ (ไม่รวม Master และดึง 5 มิติ)
def fetch_realtime_data_multivar():
    ...
    for key, node in NODES.items():
        ...
        if "Ready" in status_out and "NotReady" not in status_out:
            ...
            # 🚨 กรองเฉพาะ Worker (ตัด Master ทิ้ง 100% ตรงนี้)
            if key.startswith('worker'):
                active_workers += 1
                
                # 1. ดึง CPU & Mem Request ของ Worker เท่านั้น
                # ใช้ grep -A 6 เผื่อบรรทัดมันยาว แล้ววนลูปหาคำว่า cpu กับ memory ชัวร์สุด
                ...
                # 2. ดึง CPU & Mem Capacity ของ Worker เท่านั้น
                ...
                # 3. ดึง CPU Usage จริง ของ Worker เท่านั้น
    ...
    # 4. ดึง Pending Pods เฉพาะของแอปเรา (-n default)
    pending_count = 0
    try:
    ...
    # 5. ดึง Running Pods เฉพาะของแอปเรา (-n default)
    running_count = 0
    try:
    ...
    # คืนค่ากลุ่มแรกเป็น Tuple (5 ตัวแปรสำหรับ AI) + ค่าอื่นๆ สำหรับ Rule-based
