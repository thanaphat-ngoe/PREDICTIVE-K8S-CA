...
def main():
    ...
    while True: 
        ...
        # ==========================================
        # PHASE 4: COOLDOWN HOLD ❄️ (แช่ 15 นาที)
        # ==========================================
        log(f"\n❄️ Reached {MIN_REPLICAS} pods. Starting COOLDOWN HOLD for 15 minutes...")
        for i in range(15): 
            if random.randint(1, 100) <= 20: current_replicas = MIN_REPLICAS + 1
            else: current_replicas = MIN_REPLICAS
            scale_deployment(current_replicas)
            time.sleep(60)

        log("ส่งสัญญาณให้ Predictor เปลี่ยน Parameter")
