...
def main():
    ...
    while True:
        ...
        # ==========================================
        # 🚨 จุดที่ 2: เปลี่ยนจากพักแป๊บเดียว เป็น "แช่แข็ง (COOLDOWN HOLD)"
        # ==========================================
        log(f"❄️ Reached {MIN_REPLICAS} pods. Starting COOLDOWN HOLD for 5 minutes...")
        
        # วนลูปแช่แข็ง 5 นาที เพื่อให้ได้กราฟเส้นตรงโชว์ว่าทำงานที่ 1 เครื่องได้เสถียร
        for i in range(5): 
            print(f"⏳ แช่แข็งที่ {MIN_REPLICAS} pods... (เหลือเวลาอีก {5-i} นาที)")
            time.sleep(60)

        log("🎉 Cycle Finished. Testing completed!")
        
        # 🚨 ใส่ return เพื่อสั่งให้ "จบโปรแกรม" ไม่ต้องวนลูปเริ่มวันใหม่แล้ว
        # (เพราะการทำ Benchmark เราต้องการวัดแค่ 1 รอบเท่านั้น)
        return 
